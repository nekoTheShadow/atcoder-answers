...
func exec(stdin *Stdin, stdout *Stdout) {
	...
	if len(c) == 1 {
		x := f(c)
		if x <= 20 {
			stdout.Println(strings.Repeat("a", x))
		} else {
			stdout.Println(strings.Repeat("a", x-20) + g(20))
		}
		return
	}
	...
	if len(d) == 1 {
		...
		sum := len(c) * f(ch)
		if ch == "z" {
			stdout.Println(strings.Repeat("y", sum/25) + g(sum%25))
		} else {
			stdout.Println(strings.Repeat("z", sum/26) + g(sum%26))
		}
	} else {
		a := []rune(c)
		for {
...
func f(s string) int {
	return strings.Index("abcdefghijklmnopqrstuvwxyz", s) + 1
}

func g(i int) string {
	return "abcdefghijklmnopqrstuvwxyz"[i-1 : i]
}
