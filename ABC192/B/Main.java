...
public class Main {
    public void exec() {
        ...
        boolean ok1 = IntStream.range(0, s.length()).filter(i -> i%2==0).allMatch(i -> Character.isLowerCase(s.charAt(i)));
        boolean ok2 = IntStream.range(0, s.length()).filter(i -> i%2!=0).allMatch(i -> Character.isUpperCase(s.charAt(i)));
        if (ok1 && ok2) {
            stdout.println("Yes");
        } else {
            stdout.println("No");
