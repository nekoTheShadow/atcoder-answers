...
public class Main {
    public void exec() {
        ...
        events.add(new long[] {0, 0});
        for (int i = 0; i < n; i++) {
            long l = stdin.nextLong();
            long r = stdin.nextLong();
            long p = stdin.nextLong();
            
            events.add(new long[] {l-c, p});
            events.add(new long[] {r, -p});
        }
        events.sort(Comparator.<long[]>comparingLong(a -> a[0]).thenComparingLong(a -> a[1]));
        
        long res = 0;
        long ans = Long.MAX_VALUE;
        for (long[] event : events) {
            res += event[1];
            if (0 <= event[0] && event[0] <= w-c) {
