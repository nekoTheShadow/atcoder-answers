...
public class Main {
    public void exec() {
        ...
        Matcher m = Pattern.compile("[A-Z][a-z]*?[A-Z]").matcher(s);
        List<String> tokens = new ArrayList<>();
        while (m.find()) {
            tokens.add(m.group());
        }
        stdout.println(tokens.stream().sorted(Comparator.comparing(String::toLowerCase)).collect(Collectors.joining()));
    }
