...
public class Main {
    public void exec() {
        int n = stdin.nextInt();
        int m = stdin.nextInt();
        List<int[]> monsters1 = new ArrayList<>();
        List<int[]> monsters2 = new ArrayList<>();
        for (int i = 0; i < n; i++) monsters1.add(stdin.nextIntArray(2));
        for (int i = 0; i < m; i++) monsters2.add(stdin.nextIntArray(2));
        ...
        for (int i = 0; i < 200; i++) {
            ...
            double[] m1 = monsters1.stream().mapToDouble(a -> a[1]-mid*a[0]).sorted().toArray();
            double[] m2 = monsters2.stream().mapToDouble(a -> a[1]-mid*a[0]).sorted().toArray();
            boolean ok1 = m1[n-1]+m1[n-2]+m1[n-3]+m1[n-4]+m1[n-5] >= 0;
            boolean ok2 = m1[n-1]+m1[n-2]+m1[n-3]+m1[n-4]+m2[m-1] >= 0;
            if (ok1 || ok2) {
                ok = mid;
            } else {
                ng = mid;
