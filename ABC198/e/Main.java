...
public class Main {
    public void exec() {
        n = stdin.nextInt();
        c = stdin.nextIntArray(n);
        nexts = IntStream.range(0, n).mapToObj(unused -> new ArrayList<Integer>()).collect(Collectors.toList());
        for (int i = 0; i < n-1; i++) {
        ...
        ans.stream().sorted().forEach(stdout::println);
    }
    ...
    private void dfs (int cur) {
        for (int nxt : nexts.get(cur)) {
            ...
            if (count.getOrDefault(c[nxt], 0)==0) ans.add(nxt+1);
            count.put(c[nxt], count.getOrDefault(c[nxt], 0)+1);
            visited[nxt] = true;
            dfs(nxt);
            count.put(c[nxt], count.getOrDefault(c[nxt], 0)-1);
        }
