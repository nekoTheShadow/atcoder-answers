...
public class Main {
    public void exec() {
        n = stdin.nextInt();
        m = stdin.nextInt();
        a = stdin.nextIntArray(n);
        prevs = IntStream.range(0, n).mapToObj(unused -> new ArrayList<Integer>()).collect(Collectors.toList());
        for (int i = 0; i < m; i++) {
    ...
    private int m;
    private int[] a;
    private List<List<Integer>> prevs;
    private int[] mins;

    public int f(int x) {
        if (mins[x] != Integer.MAX_VALUE) {
            return mins[x];
        }

        if (prevs.get(x).isEmpty()) {
            return Integer.MAX_VALUE;
        }

        int min = Integer.MAX_VALUE;
        for (int prev : prevs.get(x)) {
            min = IntStream.of(min, a[prev], f(prev)).min().getAsInt();
        }
        mins[x] = min;
        return min;
    }
