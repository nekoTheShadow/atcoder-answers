...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            d.put(a, d.getOrDefault(a, 0L) + c);
            d.put(b+1, d.getOrDefault(b+1, 0L) - c);
        }

        List<Integer> events = d.keySet().stream().sorted().collect(Collectors.toList());
        long ans = 0;
        long fee = 0;
        long last = 0;
        for (int event : events) {
            if (last != event) {
                ans += Math.min(C, fee) * (event-last);
                last = event;
            }
            fee += d.get(event);
        }
