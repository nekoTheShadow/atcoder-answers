...
public class Main {
    public void exec() {
        int n = stdin.nextInt();
        int m = stdin.nextInt();
        Set<Integer> a = IntStream.range(0, n).map(unused -> stdin.nextInt()).boxed().collect(Collectors.toSet());
        Set<Integer> b = IntStream.range(0, m).map(unused -> stdin.nextInt()).boxed().collect(Collectors.toSet());
                Set<Integer> c = new HashSet<>();
        a.stream().filter(v -> !b.contains(v)).forEach(v -> c.add(v));
        b.stream().filter(v -> !a.contains(v)).forEach(v -> c.add(v));
        String ans = c.stream().sorted().map(Objects::toString).collect(Collectors.joining(" "));
        stdout.println(ans);
    }
