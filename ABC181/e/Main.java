...
public class Main {
    public void exec() {
        int n = stdin.nextInt();
        int m = stdin.nextInt();
        long[] h = stdin.nextLongArray(n);
        long[] w = stdin.nextLongArray(m);
        Arrays.sort(h);
        ...
        for (int i = 0; i < m; i++) {
            int x = bisectRight(h, w[i]);
            if (x%2 == 0) {
                ans = Math.min(ans, left[x] + right[n-x-1] + h[x] - w[i]);
            } else {
                ans = Math.min(ans, left[x-1] + right[n-x] + w[i] - h[x-1]);
            }
        }
        stdout.println(ans);
    }
