...
public class Main {
    public void exec() {
        ...
        long a = Arrays.stream(x).map(Math::abs).sum();
        long b = Arrays.stream(x).map(i -> i*i).sum();
        long c = Arrays.stream(x).map(Math::abs).max().getAsLong();
        stdout.println(a);
        stdout.println(Math.sqrt((double)b));
        stdout.println(c);
    }
