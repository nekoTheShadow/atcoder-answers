...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                if (i+1 < h  && s[i+1][j]=='.') a[i+1][j] += a[i][j];
                if (j+1 < w  && s[i][j+1]=='.') c[i][j+1] += c[i][j];
            }
        }
        for (int i = h-1; i >= 0; i--) {
            for (int j = w-1; j >= 0; j--) {
                if (i-1 >= 0 && s[i-1][j]=='.') b[i-1][j] += b[i][j];
                if (j-1 >= 0 && s[i][j-1]=='.') d[i][j-1] += d[i][j];
            }
        }

        long mod = 1_000_000_007;
        long[] modPow = new long[k+1];
        modPow[0] = 1;
        for (int i = 0; i < k; i++) {
            modPow[i+1] = (modPow[i]*2) % mod;
        }
