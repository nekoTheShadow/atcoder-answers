...
public class Main {
    public void exec() {
        ...
        double gx = stdin.nextDouble();
        double gy = stdin.nextDouble();
        double ans = f(sx, sy, gx, gy);
        stdout.println(ans);
    }

    public double f(double sx, double sy, double gx, double gy) {
        if (sx > gx) {
            return f(gx, gy, sx, sy);
        }
        return sx + Math.abs(gx-sx) / (sy+gy) * sy;
    }
