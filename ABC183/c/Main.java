...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                t[i][j] = stdin.nextLong();
            }
        }

        int ans = 0;
        List<Integer> src = IntStream.range(1, n).boxed().collect(Collectors.toList());
        for (List<Integer> indexes : new Permutations<>(src)) {
            long sum = t[0][indexes.get(0)];
            for (int i = 0; i < indexes.size()-1; i++) {
                sum += t[indexes.get(i)][indexes.get(i+1)];
            }
            sum += t[indexes.get(indexes.size()-1)][0];
            if (sum==k) ans++;
        }
        stdout.println(ans);
    }
