...
public class Main {
    ...
    public class UnionFind {
        private int[] parent;
        private List<Map<Integer, Integer>> count; // count[parent][group]

        public UnionFind(int n, int[] group) {
            this.parent = IntStream.range(0, n).toArray();
            this.count = new ArrayList<>();
            for (int i = 0; i < n; i++) {
                this.count.add(new HashMap<>());
                this.count.get(i).put(group[i], 1);
            }
        }
        ...
        public void union(int x, int y) {
            ...
            if (count.get(x).size() < count.get(y).size()) {
                parent[x] = y;
                for (Integer key : count.get(x).keySet()) {
                    count.get(y).put(key, count.get(y).getOrDefault(key, 0) + count.get(x).get(key));
                }
            } else {
                parent[y] = x;
                for (Integer key : count.get(y).keySet()) {
                    count.get(x).put(key, count.get(x).getOrDefault(key, 0) + count.get(y).get(key));
                }
            }
        }

        public int count(int x, int y) {
            return count.get(find(x)).getOrDefault(y, 0);
        }
    }
