...
public class Main {
    ...
    public void exec() {
        ...
        PriorityQueue<Tuple> q = new PriorityQueue<>(Comparator.comparingLong(t -> t.x*-1));
        for (int i = 0; i < n; i++) {
            q.offer(new Tuple(i, b[i], true));
        }
        ...
        for (int i = 0; i < k; i++) {
            ...
            ans += t.x;
            if (t.isB) {
                q.offer(new Tuple(t.i, a[t.i]-b[t.i], false));
            }
        }
        stdout.println(ans);
    }
    ...
    private class Tuple {
        private int i; // 問題番号
        private long x; // 点数
        private boolean isB; // 部分点かどうか
        
        public Tuple(int i, long x, boolean isB) {
            super();
            this.i = i;
            this.x = x;
            this.isB = isB;
