...
public class Main {
    public void exec() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j <= 7; j++) {
                dp[i+1][j] += dp[i][j];
                if (s.charAt(i)=='a' && j==0) dp[i+1][j+1] += dp[i][j];
                if (s.charAt(i)=='t' && j==1) dp[i+1][j+1] += dp[i][j];
                if (s.charAt(i)=='c' && j==2) dp[i+1][j+1] += dp[i][j];
                if (s.charAt(i)=='o' && j==3) dp[i+1][j+1] += dp[i][j];
                if (s.charAt(i)=='d' && j==4) dp[i+1][j+1] += dp[i][j];
                if (s.charAt(i)=='e' && j==5) dp[i+1][j+1] += dp[i][j];
                if (s.charAt(i)=='r' && j==6) dp[i+1][j+1] += dp[i][j];
                dp[i+1][j] %= mod;
                if (j+1<8) dp[i+1][j+1] %= mod; 
            }
        }
        stdout.println(dp[n][7]);
    }
