...
public class Main {
    ...
    public void exec() {
        ...
        long ymax = Arrays.stream(y).max().getAsLong();
        long xmin = Arrays.stream(x).min().getAsLong();
        long ymin = Arrays.stream(y).min().getAsLong();
        
        
        for (int i = 0; i < Q; i++) {
            int q = stdin.nextInt()-1;
            long ans = LongStream.of(Math.abs(x[q]-xmax), Math.abs(x[q]-xmin), Math.abs(y[q]-ymax), Math.abs(y[q]-ymin)).max().getAsLong();
            stdout.println(ans);
        }
    }
