...
public class Main {
    public void exec() {
        ...
        List<TreeSet<Integer>> p = IntStream.range(0, 26).mapToObj(unused -> new TreeSet<Integer>()).collect(Collectors.toList());
        for (int i = 0; i < n; i++) p.get(s.charAt(i) - 'a').add(i);
        ...
        while (ans.length() < k) {
            for (char ch = 'a'; ch <= 'z'; ch++) {
                Integer idx = p.get(ch-'a').higher(cur);
                if (idx!=null && ans.length()+n-idx >= k) {
                    ans.append(ch);
                    cur = idx;
                    break;
                }
