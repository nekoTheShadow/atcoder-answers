...
public class Main {
    ...
    private int[][] diffs = new int[][] {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
    private boolean[][] used;
    
    public void exec() {
        this.h = stdin.nextInt();
        this.w = stdin.nextInt();
        this.c = new char[h][];
        for (int i = 0; i < h; i++) c[i] = stdin.nextString().toCharArray();
        this.used = new boolean[h][w];
        
        int ans = Integer.MIN_VALUE;
        for (int x = 0; x < h; x++) {
            for (int y = 0; y < w; y++) {
                ans = Math.max(dfs(x, y, x, y), ans);
            }
        }
    ...
    private int dfs(int startX, int startY, int x, int y) {
        if (startX==x && startY == y && used[x][y]) return 0;
        ...
        for (int[] diff : diffs) {
            ...
            if (0<=nx && nx<h && 0<=ny && ny<w && c[nx][ny]=='.') {
                if ((startX==nx && startY==ny) || !used[nx][ny]) {
                    int v = dfs(startX, startY, nx, ny);
